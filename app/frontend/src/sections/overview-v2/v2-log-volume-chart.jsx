...
export function V2LogVolumeChart({ title, subheader, info, chart, sx }) {
  const t = useMonitoringTokens();

  const chartColors = chart?.colors ?? [t.chart.info, t.chart.warn, t.chart.error];
  ...
  return (
    ...
    >
      ...
      <Box sx={{ px: 1 }}>
        <Chart type="area" series={chart?.series ?? []} options={chartOptions} sx={{ height: 320 }} />
      </Box>
    </Box>
  );
}
