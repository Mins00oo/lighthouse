...
import { useGetRecentErrors } from 'src/actions/monitoring';

import { V2InfoTip } from 'src/sections/overview-v2/v2-info-tip';

// ----------------------------------------------------------------------

const INFO = '최근 발생한 예외(Exception) 목록과 발생 빈도. 빈도순 정렬';
...
export function WidgetRecentErrors({ from, to }) {
  const t = useMonitoringTokens();
  const { recentErrors } = useGetRecentErrors(from, to);
  const sorted = [...(recentErrors ?? [])].sort((a, b) => (b.count ?? 0) - (a.count ?? 0));
      ...
      <Box sx={{ flex: 1, overflow: 'auto' }}>
        {sorted.map((row, index) => (
          ...
          >
            ...
            <Box sx={{ width: 65, textAlign: 'right', fontVariantNumeric: 'tabular-nums' }}>
              <Box
                component="span"
                sx={{
                  fontSize: '0.75rem',
